...
const double Ebeam = 6.405; // beam energy is GeV
const double Mp = 0.93827; // proton mass in GeV
const double Me = 0.000511; // electron mass in GeV
const double normfac = 0.964538E+07; //SIMC normfac
const double sfac = 211.5;//scale factor for the charge, effs of data: SIMC = 1mC and 100% eff.
const double theta_ee = -27.62*3.141592/180.; //SHMS angle
const double theta_pp = 27.5*3.141592/180.; //HMS angle
const double ep_cen = 3.609; //SHMS central momentum
const double pp_cen = 3.609; //HMS central momentum  
const char* filecD = "ep_1854.root"; // Data filename
const char* filecM = "h1_3.root"; // SIMC file name
const char* filecO = "run_1854_kine.pdf"; //output pdf file name


void Plot_kine_compare_eSHMS(){

 //read the input file from data
  TString fileNameD = ROOT_FILE_PATH;
  fileNameD += filecD; //read the root file from data
  ...
  fileNameM += filecM; //read the root file from SIMC
  ...
  for (int kk=0; kk<nentriesD; kk++){
    ...
    double hpp = tt->GetLeaf("H.gtr.p")->GetValue();
    double deltap = tt->GetLeaf("H.gtr.dp")->GetValue();
    double pkinPm = tt->GetLeaf("H.kin.secondary.pmiss")->GetValue();
    double pxptar = tt->GetLeaf("H.gtr.ph")->GetValue();
    double pyptar = tt->GetLeaf("H.gtr.th")->GetValue();


    double pcalepr = tt->GetLeaf("P.cal.eprtracknorm")->GetValue();
    double pcaletot = tt->GetLeaf("P.cal.etottracknorm")->GetValue();
    double pcernpe = tt->GetLeaf("P.hgcer.npeSum")->GetValue();
    double pbeta = tt->GetLeaf("P.gtr.beta")->GetValue();
    double pep = tt->GetLeaf("P.gtr.p")->GetValue();
    double deltae = tt->GetLeaf("P.gtr.dp")->GetValue();
    double pkinW = tt->GetLeaf("P.kin.primary.W")->GetValue();
    double exptar = tt->GetLeaf("P.gtr.ph")->GetValue();
    double eyptar = tt->GetLeaf("P.gtr.th")->GetValue();
    ...
    Double_t p_e, p_p, esphi, psphi, Pm_x, Pm_y; //variable of momentum range for e and p

    if (pbeta>0.5 && pbeta<1.5 && hbeta>0.6 && hbeta<1.3 && pcernpe>0.1 && hcernpe<0.1 && pcaletot >0.8 && pcaletot<1.3 && pcalepr>0.02) {
      ...
      else
	{
      ...
      esphi = atan(p_ey/p_ex);      
      psphi = atan(p_py/p_px);
      ...
      W_2 = sqrt(Mp * Mp + 2 * Mp * (E_corr - Ep) - Q_2);           
      Em = E_corr + Mp - (Ep + Epp); // missing Energy.
      Pm_par = q_z - p_pz; //  missing parallel momentum
      Pm_x = q_x - p_px; 
      Pm_y = q_y - p_py;
      Pm_per = sqrt(Pm_x*Pm_x + Pm_y*Pm_y);// missing perpendicular momentum.
  ...
  for (int kk=0; kk<nentriesM; kk++){
    ...
    double normwt = wt*normfac*sfac/nentriesM;
