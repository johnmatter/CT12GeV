...
void CTemiss(Int_t firstRun, Int_t lastRun, TString SimcFileName, TString SimcFileNameNor, TString target)
{
   ...
   //-----These offsets (for data) to be removed once SHMS is better calibrated ----------
    ...
    int cnts = 0;
    Double_t ebeam=10.58792;//6.43;//10.589;//6.43
    /*
    //--Q2==11.5
    Double_t etheta=-28.505*3.141592/180.;
    Double_t ptheta=17.83*3.141592/180.;
    Double_t eP0=4.478*0.9945;
    Double_t pP0=7.001*0.985*0.9965;
    */
    /*
    //Q2=9.5, sm coll
    Double_t etheta=-23.186*3.141592/180.;
    Double_t ptheta=21.625*3.141592/180.;
    Double_t eP0=5.539*0.9898;
    Double_t pP0=5.925*0.985*0.997;
    */
    
    //Q2=9.5, lg coll
    Double_t etheta=-23.19*3.141592/180.;
    Double_t ptheta=21.595*3.141592/180.;
    Double_t eP0=5.539*0.9942;
    Double_t pP0=5.925*0.985*0.997;
     
    /*
    //Q2=14.3, sm coll
    Double_t etheta=-39.28*3.141592/180.;
    Double_t ptheta=12.8*3.141592/180.;
    Double_t eP0=2.982;
    Double_t pP0=8.505*0.985;
    */
    /*
    //Q2=14.3, lg coll
    Double_t etheta=-39.28*3.141592/180.;
    Double_t ptheta=12.8*3.141592/180.;
    Double_t eP0=2.982*0.997;
    Double_t pP0=8.505*0.985*0.996;
    */
    /*
    //Q2=8, sm coll
    Double_t etheta=-45.11*3.141592/180.;
    Double_t ptheta=17.12*3.141592/180.;
    Double_t eP0=2.131*0.998;
    Double_t pP0=5.122*0.985*0.998;
    */
    ...
    TH1D *pSimcEmN = new TH1D("pSimcEmN","Missing Energy (GeV)",200,-0.15,0.25);
    ...
    pSimcEmN->SetMarkerColor(kGreen);  
	...
	tree->GetEntry(event);
	//	if (ct->fP_gtr_dp<0 || ct->fP_gtr_dp>5) {continue;}
    ...
    ////////////////////////////////////
    //------------------ Plot from Simc --------------------------
    CTRun *ctt = new CTRun(firstRun,lastRun);

    ctt->AddSimc(SimcFileNameNor);

    if(!ctt->fSimcExist)
	return;
    
    TTree *simc2 = ctt->GetSimcTree();

    Int_t nEventsSimc2 = simc2->GetEntries();

    for (Int_t event = 0; event < nEventsSimc2; ++event)
    {
	if(event%50000 == 0)
	    cout << (Int_t)(100*event/nEventsSimc2) <<" % Done"<< endl;

	simc2->GetEntry(event);
	//Kinematic Cut
	if( !(ctt->fHSdelta > -10 && ctt->fHSdelta < 10
	      && ctt->fPSdelta > -15 && ctt->fPSdelta < 15))
	    continue;	
	if(target == "c")       // For carbon-12
	{
	    if(!(fabs(ctt->fPm)<0.4))                                    // Kinematic cut
		continue;
	}
	else if(target == "h")  // For hydrogen-1
	{
	    if(!(ctt->fW>=0.75 && ctt->fW<=1.15))                // Kinematic cut
		continue;	    
	}

	//------ Kinematic quanties ---------
	pSimcEmN->Fill(ctt->fEm, ctt->fWeight);	
    }
    ...
    pSimcEmN->DrawNormalized("prof same");
    c3->cd(3);
