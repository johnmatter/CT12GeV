...
void CTEmTest_h(Int_t firstRun, Int_t lastRun, TString SimcFileName)
{
   ...
   //-----These offsets (for data) to be removed once SHMS is betetr calibrated ----------
    ...
    TH2D *Emhdelta = new TH2D("Emhdelta","Em vs HMS delta; HMS delta (%); Em (GeV)",100,-12,12,100,-0.15,0.25);
    TH2D *Empdelta = new TH2D("Empdelta","Em vs SHMS delta; SHMS delta (%); Em (GeV)",100,-17,17,100,-0.15,0.25);
    TH2D *Emhtheta = new TH2D("Emhtheta","Em vs HMS yptar; HMS yptar (rad); Em (GeV)",100,-0.05,0.05,100,-0.15,0.25);
    TH2D *Emptheta = new TH2D("Emptheta","Em vs SHMS yptar; SHMS yptar (rad); Em (GeV)",100,-0.05,0.05,100,-0.15,0.25);
    TH2D *Emhxptar = new TH2D("Emhxptar","Em vs xptar; HMS xptar (rad); Em (GeV)",100,-0.1,0.1,100,-0.15,0.25);
    TH2D *Empxptar = new TH2D("Empxptar","Em vs xptar; SHMS xptar (rad); Em (GeV)",100,-0.1,0.1,100,-0.15,0.25);
    TH2D *Empxfp = new TH2D("Empxfp","Em vs xfp; SHMS xfp (cm); Em (GeV)",100,-20,20,100,-0.15,0.25);
    TH2D *Empxpfp = new TH2D("Empxpfp","Em vs xpfp; SHMS xpfp (rad); Em (GeV)",100,-0.06,0.06,100,-0.15,0.25);
    TH2D *Empyfp = new TH2D("Empyfp","Em vs yfp; SHMS yfp (cm); Em (GeV)",100,-15,15,100,-0.15,0.25);
    TH2D *Empypfp = new TH2D("Empypfp","Em vs ypfp; SHMS ypfp (rad); Em (GeV)",100,-0.035,0.035,100,-0.15,0.25);
    TH2D *Emhxfp = new TH2D("Emhxfp","Em vs xfp; HMS xfp (cm); Em (GeV)",100,-25,25,100,-0.15,0.25);
    TH2D *Emhxpfp = new TH2D("Emhxpfp","Em vs xpfp; HMS xpfp (rad); Em (GeV)",100,-0.065,0.065,100,-0.15,0.25);
    TH2D *Emhyfp = new TH2D("Emhyfp","Em vs yfp; HMS yfp (cm); Em (GeV)",100,-15,20,100,-0.15,0.25);
    TH2D *Emhypfp = new TH2D("Emhypfp","Em vs ypfp; HMS ypfp (rad); Em (GeV)",100,-0.035,0.035,100,-0.15,0.25);
    ...
    //SIMC
    //Reconstructed quantities
    TH2D *sEmhdelta = new TH2D("sEmhdelta","SIMC Em vs HMS delta; HMS delta (%); Em (GeV)",100,-12,12,100,-0.15,0.25);
    TH2D *sEmpdelta = new TH2D("sEmpdelta","SIMC Em vs SHMS delta; SHMS delta (%); Em (GeV)",100,-17,17,100,-0.15,0.25);
    TH2D *sEmhtheta = new TH2D("sEmhtheta","SIMC Em vs HMS yptar; HMS yptar (rad); Em (GeV)",100,-0.05,0.05,100,-0.15,0.25);
    TH2D *sEmptheta = new TH2D("sEmptheta","SIMC Em vs SHMS yptar; SHMS yptar (rad); Em (GeV)",100,-0.05,0.05,100,-0.15,0.25);
    TH2D *sEmhxptar = new TH2D("sEmhxptar","SIMC Em vs xptar; HMS xptar (rad); Em (GeV)",100,-0.1,0.1,100,-0.15,0.25);
    TH2D *sEmpxptar = new TH2D("sEmpxptar","SIMC Em vs xptar; SHMS xptar (rad); Em (GeV)",100,-0.1,0.1,100,-0.15,0.25);
    TH2D *sEmpxfp = new TH2D("sEmpxfp","Em vs SHMS xfp; SHMS xfp (cm); Em (GeV)",100,-20,20,100,-0.15,0.25);
    TH2D *sEmpxpfp = new TH2D("sEmpxpfp","Em vs SHMS xpfp; SHMS xpfp (rad); Em (GeV)",100,-0.06,0.06,100,-0.15,0.25);
    TH2D *sEmpyfp = new TH2D("sEmpyfp","Em vs SHMS yfp; SHMS yfp (cm); Em (GeV)",100,-15,15,100,-0.15,0.25);
    TH2D *sEmpypfp = new TH2D("sEmpypfp","Em vs SHMS ypfp; SHMS ypfp (rad); Em (GeV)",100,-0.035,0.035,100,-0.15,0.25);
    TH2D *sEmhxfp = new TH2D("sEmhxfp","Em vs HMS xfp; HMS xfp (cm); Em (GeV)",100,-25,25,100,-0.15,0.25);
    TH2D *sEmhxpfp = new TH2D("sEmhxpfp","Em vs HMS xpfp; HMS xpfp (rad); Em (GeV)",100,-0.065,0.065,100,-0.15,0.25);
    TH2D *sEmhyfp = new TH2D("sEmhyfp","Em vs HMS yfp; HMS yfp (cm); Em (GeV)",100,-15,20,100,-0.15,0.25);
    TH2D *sEmhypfp = new TH2D("sEmhypfp","Em vs HMS ypfp; HMS ypfp (rad); Em (GeV)",100,-0.035,0.035,100,-0.15,0.25);
	...
	if( !(ct->fP_gtr_beta > 0.6 && ct->fP_gtr_beta < 1.4                             // proton beta cut
	      ...
	      && ct->fH_kin_primary_W < 1.15 && ct->fH_kin_primary_W > 0.75 ))
	    continue;

	//--- reconstructed quantities ---
        Emhdelta->Fill(ct->fH_gtr_dp,ct->fP_kin_secondary_emiss);
        Empdelta->Fill(ct->fP_gtr_dp,ct->fP_kin_secondary_emiss);
        Emhtheta->Fill(ct->fH_gtr_ph,ct->fP_kin_secondary_emiss);
        Emptheta->Fill(ct->fP_gtr_ph,ct->fP_kin_secondary_emiss);
        Emhxptar->Fill(ct->fH_gtr_th,ct->fP_kin_secondary_emiss);
        Empxptar->Fill(ct->fP_gtr_th,ct->fP_kin_secondary_emiss);
        Empxfp->Fill(ct->fP_dc_x_fp,ct->fP_kin_secondary_emiss);
        Empxpfp->Fill(ct->fP_dc_xp_fp,ct->fP_kin_secondary_emiss);
        Empyfp->Fill(ct->fP_dc_y_fp,ct->fP_kin_secondary_emiss);
        Empypfp->Fill(ct->fP_dc_yp_fp,ct->fP_kin_secondary_emiss);
        Emhxfp->Fill(ct->fH_dc_x_fp,ct->fP_kin_secondary_emiss);
        Emhxpfp->Fill(ct->fH_dc_xp_fp,ct->fP_kin_secondary_emiss);
        Emhyfp->Fill(ct->fH_dc_y_fp,ct->fP_kin_secondary_emiss);
        Emhypfp->Fill(ct->fH_dc_yp_fp,ct->fP_kin_secondary_emiss);
