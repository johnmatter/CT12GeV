...
void CTdataVsSimc(Int_t firstRun, Int_t lastRun, TString SimcFileName, TString target)
{
   ...
   //-----These offsets (for data) to be removed once SHMS is betetr calibrated ----------
    ...
    TH1D *sSimcorsi = new TH1D("sSimcorsi","P_shms - P' (GeV); p_mom diff(GeV); Normalized counts", 150, -1.5, 1.5);
	...
	if(target == "h")
	  {
	    pEm->Fill(ct->fP_kin_secondary_emiss + OffsetEm);
            ...
            htheta = acos((cos(etheta) + (ct->fH_gtr_th)*sin(etheta))/r);
	...
	if (target == "c")
        ...
        if (target == "h")
	 {
	   pSimcPm->GetXaxis()->SetRangeUser(-0.15,0.25);
           ...
           ssim_corsi = psimP - 2.*mass_p*ebeam*cos(ssimtheta)/(ebeam+mass_p)/(1-pow(ebeam*cos(ssimtheta)/(ebeam+mass_p),2));        sSimcorsi->Fill(ssim_corsi,ct->fWeight);
