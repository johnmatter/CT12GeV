...
void Bean_counter(Int_t runNumber){
 ...
 //read the input file from data
  ...
  tt->SetBranchAddress("T.coin.hTRIG4_ROC2_tdcTimeRaw", &TcoinhTRIG4_ROC2_tdcTimeRaw);                   
   ...
   Wcut = "H.kin.primary.W >0.8 && H.kin.primary.W < 1.1";
  ...
  for (int kk=0; kk<nentriesD; kk++){
    ...
    if (kk % 50000 == 0) cout << kk*100/nentriesD << "   % of data done" << endl;
      ...
      if (pbeta>0.8 && pbeta<1.3 && hbeta>0.8 && hbeta<1.2 && pcernpe<0.1 && hcernpe>0.1 && hcaletot >0.8 && hcaletot<1.2 && PhodStatus == 1 && HhodStatus ==1 && hdelta > -10 && hdelta < 10 && pdelta > -12 && pdelta < 12 && pkinW > 0.75 && pkinW < 1.15) 
      { 
  ...
  can1->cd(1);
  h_hdelta->Draw();  
  h_hdelta->GetXaxis()->SetTitle("HMS delta (%)");            
  can1->cd(2);
  h_hxptar->Draw();  
  can1->cd(3);
  h_hyptar->Draw();  
