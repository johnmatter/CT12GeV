...
void dc_cal_shms(Int_t runNo, Int_t eventNo)
{
  ...
  TCanvas *c1 = new TCanvas("c1", "SHMS Drift Distance", 1000, 1000);
  c1->Divide(2,6);
  for(int j=0; j<12; j++){
    c1->cd(j+1);
    drift_dist[j]->Draw();
    c1->SaveAs("drift_dist_shms.pdf");
   
  }
 
  TCanvas *c2 = new TCanvas("c2", "SHMS Drift Time", 1000, 1000);
  c2->Divide(2,6); 
  for(int j=0; j<12; j++){
    c2->cd(j+1);
    drift_time[j]->Draw();
    c2->SaveAs("drift_time_shms.pdf");
